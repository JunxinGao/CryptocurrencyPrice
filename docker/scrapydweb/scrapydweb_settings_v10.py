...
PASSWORD = os.environ.get('PASSWORD', 'public')
...
#   - e.g. ('', '', '127.0.0.1', '6800', '') or ('username', 'password', 'localhost', '6801', 'group')
SCRAPYD_SERVER = os.environ.get('SCRAPYD_SERVER', '127.0.0.1:6800')
SCRAPYD_SERVERS = [SCRAPYD_SERVER]
# [
#     'pdgzf:6800' if os.environ.get('PROD_ENV') == 'prod' else '127.0.0.1:6800',
#     # 'username:password@localhost:6801#group',
#     # ('username', 'password', 'pdgzf' if os.environ.get('PROD_ENV') == 'prod' else 'localhost', '6801', 'group'),
# ]
...
# The default is ['.logs', '.logs.gz', '.txt'].
SCRAPYD_LOG_EXTENSIONS = ['.logs', '.logs.gz', '.txt']
...
# e.g. ['username@gmail.com', ]
EMAIL_RECIPIENTS = os.environ.get('EMAIL_RECIPIENTS', [EMAIL_SENDER])
...
SMTP_PORT = os.environ.get('SMTP_PORT', 0)
SMTP_OVER_SSL = os.environ.get('SMTP_OVER_SSL', False)
# The timeout in seconds for the connection attempt, the default is 30.
SMTP_CONNECTION_TIMEOUT = os.environ.get('SMTP_CONNECTION_TIMEOUT', False)
...
########## advanced triggers ##########
# - LOG_XXX_THRESHOLD:
#   - Trigger alert the first time reaching the threshold for a specific kind of logs.
#   - The default is 0, set it to a positive integer to enable this trigger.
...
DATA_PATH = os.environ.get('DATA_PATH', '/scrapydweb_data')
